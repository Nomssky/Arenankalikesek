...
export default function Hero({ title, subtitle, image, height = 'md', children }: HeroProps) {
  ...
  return (
    <section
      className={`page-hero relative ${heights[height]} flex items-center justify-center overflow-hidden bg-emerald-950 text-white`}
    >
      <div
      ...
      <div className={`page-hero__content relative z-10 container-page text-center ${isLargeHero ? 'pb-14 pt-24' : 'pb-12 pt-24'}`}>
        ...
        {subtitle && (
          <p className="page-hero__subtitle text-sm md:text-base leading-7 text-white/85 max-w-2xl mx-auto mt-5 mb-8 text-balance">
            {subtitle}
          </p>
        )}
        {children}
      </div>
      <div className="page-hero__scroll absolute bottom-6 left-1/2 z-10 -translate-x-1/2 text-center text-[10px] font-medium tracking-[0.24em] text-white/60 uppercase">
